...
class Monikit_App_Gdpr_User_Data_Deletion_API {
	...
	public function register_api_routes() {
		...
		register_rest_route( 'monigpdr/v1', '/delete/contact', array(
			'methods' => 'POST',
			'callback' => array( $this, 'handle_contact_deletion_request' ),
			'permission_callback' => '__return_true',
			'args' => array(
				'headers' => array(
					'validate_callback' => function( $param, $request, $key ) {
	...
	/**
	 * Handle Contact user deletion request with OAuth2 access token
	 *
	 * @access	public
	 * @since	1.5.0
	 * @param	WP_REST_Request	$request	Request object
	 * @return	WP_REST_Response	Response object
	 */
	public function handle_contact_deletion_request( $request ) {
		// Basic rate limiting protection
		$client_ip = $this->get_client_ip();
		$rate_limit_key = 'monikit_api_rate_limit_' . md5( $client_ip );
		$rate_limit_count = get_transient( $rate_limit_key );
		...
		// Check if Contact realm is configured
		if ( empty( $settings['keycloak_base_url'] ) || 
			 empty( $settings['keycloak_contact_realm'] ) || 
			 empty( $settings['keycloak_client_id'] ) || 
			 empty( $settings['keycloak_client_secret'] ) || 
			 empty( $settings['keycloak_admin_username'] ) || 
			 empty( $settings['keycloak_admin_password'] ) ) {
		...
		try {
			// Validate access token and get user info - using Contact realm
			$user_info = $this->validate_contact_access_token( $access_token, $settings );
			...
			MONIGPDR()->logs->log_action( 
				...
				'request', 
				'pending', 
				'Contact access token deletion request received', 
				array(
					'source' => 'access_token',
					'realm' => 'Contact',
					'ip_address' => $this->get_client_ip(),
			...
			// Delete user from Contact realm in Keycloak
			$deletion_result = $this->delete_contact_user_by_token_info( $user_id, $user_email, $settings );

			if ( $deletion_result['success'] ) {
				// Update the original request status to completed
				MONIGPDR()->logs->update_log_status( $log_email, 'request', 'completed', 'Contact access token deletion request completed successfully.' );
				...
				MONIGPDR()->logs->log_action( 
					...
					'deletion', 
					'success', 
					'Contact account successfully deleted via access token', 
					array(
						'source' => 'access_token',
						'realm' => 'Contact',
						'keycloak_user_id' => $deletion_result['keycloak_user_id'] ?? $user_id,
				...
				if ( strpos( $deletion_result['message'], 'not found' ) !== false || 
					...
					MONIGPDR()->logs->log_action( 
						...
						'request', 
						'failed', 
						'Contact user not found via access token', 
						array(
							'source' => 'access_token',
							'realm' => 'Contact',
							'ip_address' => $this->get_client_ip(),
				...
				MONIGPDR()->logs->log_action( 
					...
					'request', 
					'failed', 
					'Contact access token deletion failed: ' . $deletion_result['message'], 
					array(
						'source' => 'access_token',
						'realm' => 'Contact',
						'ip_address' => $this->get_client_ip(),
			...
			MONIGPDR()->logs->log_action( 
				...
				'request', 
				'failed', 
				'Contact access token deletion exception: ' . $e->getMessage(), 
				array(
					'source' => 'access_token',
					'realm' => 'Contact',
					'ip_address' => $this->get_client_ip(),
	...
	/**
	 * Validate Contact access token using Keycloak userinfo endpoint
	 *
	 * @access	private
	 * @since	1.5.0
	 * @param	string	$access_token	Access token to validate
	 ...
	 * @return	array|false	User info array or false if invalid
	 */
	private function validate_contact_access_token( $access_token, $settings ) {
		$base_url = rtrim( $settings['keycloak_base_url'], '/' );
		$contact_realm = $settings['keycloak_contact_realm'];
		...
		$userinfo_url = $base_url . '/realms/' . $contact_realm . '/protocol/openid-connect/userinfo';
		...
		if ( $curl_error ) {
			error_log( 'Monikit GDPR: cURL error validating Contact access token: ' . $curl_error );
			return false;
		}
		
		if ( $http_code !== 200 ) {
			error_log( 'Monikit GDPR: Invalid Contact access token, HTTP code: ' . $http_code );
			return false;
		}
		
		$user_info = json_decode( $response, true );
		if ( ! $user_info || ! is_array( $user_info ) ) {
			error_log( 'Monikit GDPR: Invalid Contact userinfo response format' );
			return false;
		}
	...
	/**
	 * Delete Contact user from Keycloak using user ID or email
	 *
	 * @access	private
	 * @since	1.5.0
	 * @param	string	$user_id		Keycloak user ID
	 ...
	 * @return	array	Result array
	 */
	private function delete_contact_user_by_token_info( $user_id, $user_email, $settings ) {
		// Get admin token for Contact realm operations
		$admin_token = $this->get_keycloak_admin_token( $settings );
		if ( ! $admin_token ) {
			return array(
				'success' => false,
		...
		// If we have user ID, delete directly
		if ( ! empty( $user_id ) ) {
			$delete_url = $base_url . '/admin/realms/' . $contact_realm . '/users/' . $user_id;
			...
			if ( $http_code === 204 ) {
				...
				error_log( 'Monikit GDPR: Contact user deletion failed, HTTP code: ' . $http_code . ', Response: ' . $response );
				return array(
					'success' => false,
					'message' => 'Keycloak error, HTTP code: ' . $http_code . ', Response: ' . $response
				);
		...
		// If we only have email, search for user first
		if ( ! empty( $user_email ) ) {
			$search_url = $base_url . '/admin/realms/' . $contact_realm . '/users?email=' . urlencode( $user_email );
			...
			// Delete the found user recursively
			return $this->delete_contact_user_by_token_info( $found_user_id, null, $settings );
		}
